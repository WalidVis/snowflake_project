...
-------------------------------------------- Create stages ---------------------
create stage if not exists DEV_POC_VISEO_DB.RAW_LAYER.ARCHIVE_INTERNAL_STAGE;

create stage if not exists  DEV_POC_VISEO_DB.RAW_LAYER.LANDINGg_INTERNAL_STAGE;

create stage if not exists  DEV_POC_VISEO_DB.RAW_LAYER.ERROR_INTERNAL_STAGE;

create or replace stage DEV_POC_VISEO_DB.RAW_LAYER.EXTERNAL_AZUR_STAGE
...
create or replace table DEV_POC_VISEO_DB.BRONZE_LAYER.PRC_BENCHMARK_BRZ
(
...
create or replace table DEV_POC_VISEO_DB.BRONZE_LAYER.PRC_CUSTOMER_ERP_PRICING_MARKET_BRZ
(
...
create or replace table DEV_POC_VISEO_DB.BRONZE_LAYER.PRC_CAMPAIGN_MARKET_BRZ (
...
create or replace table DEV_POC_VISEO_DB.MONITORING_LAYER.MONITORING_INGEST
(
  file VARCHAR(200),
...
create or replace file format DEV_POC_VISEO_DB.bronze_layer.csv_file_format 
    TYPE = CSV 
...
create or replace file format DEV_POC_VISEO_DB.bronze_layer.json_file_format 
    TYPE = JSON 
