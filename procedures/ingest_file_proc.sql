CREATE OR REPLACE PROCEDURE DEV_POC_VISEO_DB.bronze_layer.ingest_file_proc(
  src_schema VARCHAR,// Absolute name (database.schema)
  target_table_abs_name VARCHAR,// Absolute name (database.schema.table_name)
  stage_name VARCHAR,// Absolute name (@database.schema.stage_name)
  stage_suffix_directory_path VARCHAR,/* suffix directory path of the stage with "/" at the end ( .../path/ ) */
  pattern_file_name VARCHAR,
  file_format VARCHAR,// Absolute name (database.schema)
  on_error_option VARCHAR )
  RETURNS table()
  LANGUAGE SQL
  EXECUTE AS CALLER //mandatory for RM command
AS
DECLARE
